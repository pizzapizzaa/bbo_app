...
describe('the price list', () => {
  ...
  it('splits add-ons into gear rental and retail stock', () => {
    ...
    expect(rentals).toEqual([
      'Shoes Rental',
      'Chalk Bag Rental (1 person)',
      'Chalk Bag Rental (2 people)',
      'Chalk Bag Rental (3+ people)',
    ]);
  });

  it('lists every chalk bag tier as its own product, cheapest first', () => {
    const tiers = ADDONS.filter(a => a.tier?.of === 'Chalk Bag Rental');
    expect(tiers.map(a => a.price)).toEqual([15_000, 20_000, 25_000]);
    // The form submits one tier by name, so each has to be billable on its own.
    for (const t of tiers) expect(isKnownAddon(t.name)).toBe(true);
  });

  it('assigns every add-on to a column the check-in form can render', () => {
    for (const a of ADDONS) expect(['Climbing', 'Food & Drinks']).toContain(a.group);
  });
...
describe('computeCheckinAmount — referral and promo codes', () => {
  ...
  it('cuts gear rental by the code’s rental percentage but never retail stock', () => {
    const p = computeCheckinAmount({
      date: ORDINARY, checkin_type: 'Day Pass – Adult', discount: 'referral', referral: CODE,
      addons: ['Shoes Rental', 'Chalk Bag Rental (1 person)', 'Pocari'],
    });

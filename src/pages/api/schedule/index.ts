...
import { ok, serverError, unauthorized, authFromRequest, requireAdmin } from '../../../lib/auth';
...
export const GET: APIRoute = async ({ request }) => {
  // Part-timers need to read the calendar to find slots to claim, so any signed-in
  // account may read; only admins may write (see POST below).
...
export const POST: APIRoute = async ({ request }) => {
  ...
  // A part-time slot created with a name on it is one the admin filled in
  // themselves — record it as claimed so it doesn't show up as open.
